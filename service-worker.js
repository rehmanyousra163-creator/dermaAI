// DermaAI Service Worker - Offline support and caching
const CACHE_NAME = 'dermai-cache-v3.3';
const urlsToCache = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('install', event => {
  console.log('[ServiceWorker] Installing DermaAI...');
  event.waitUntil(
    ...
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME && cacheName.includes('dermai')) {
...
// Background sync for offline queuing (optional)
self.addEventListener('sync', event => {
  if (event.tag === 'sync-scans') {
    console.log('[ServiceWorker] Background sync triggered');
    event.waitUntil(
      // Handle queued scans when back online
      Promise.resolve()
    );
  }
});